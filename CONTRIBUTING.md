...
# see AGENTS.md § Security for the single canonical policy description)
just install-hooks
...
`AGENTS.md` is the sole authoritative agent contract for this repository. It
defines safety boundaries for AI agent runtimes (permitted actions, prohibited
actions, escalation rules, the `--dangerously-skip-permissions` policy) and
documents developer context: naming conventions, YAML format, validators, and
the quick-start workflow. Update it when you change anything a developer needs
to know day-to-day, when you add a new operation that agents should or should
not perform, or when escalation thresholds change.

`CLAUDE.md` is a pointer to `AGENTS.md` and should not carry content of its own.
